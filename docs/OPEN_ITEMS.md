...
- Migrations 038 (`backfill_taxonomy`) and 039 (`backfill_gender_policy_from_public_data`) were added to the repo 2026-08-24 (PR #85) — repo-completeness re-adds of backfills already run against live in May/June 2026. Nothing to apply. **Correction, 2026-09-01, same day, from Codex review on PR #92**: this session initially wrote both off as "idempotent no-ops if re-run" — wrong for 038. Its last statement (`UPDATE resources SET stale_after_days = stale_after_days`, no `WHERE`) stamps `updated_at = now()` on every row via the `resources_updated_at` trigger, same hazard migration 037 explicitly guards against with `DISABLE/ENABLE TRIGGER` for the identical statement. 038 has no such guard. Fixed the migration's own header comment plus all three docs that repeated the wrong claim (`CLAUDE.md`, `README.md`, this file). 039 has no such issue — genuinely idempotent.
...
**Second follow-up pass (2026-09-01, same day) — read the two highest-consequence unread items from the first pass:**
- **`/community-voices` sitemap question: resolved, deliberate.** Read `CommunityVoicesPage.tsx` directly. `SeoHead` gets `noindex` (confirmed in `SeoHead.tsx`: renders `<meta name="robots" content="noindex, nofollow">`), and the file's own comment states the reasoning — "No fabricated testimonials — honest empty state until real stories exist" / "noindex (and kept out of sitemap.xml) until there is real content." The page currently renders an `EmptyState`, not testimonials. Not a gap. Add to the sitemap once real stories exist, not before.
- **Privacy Policy and Terms of Use: found a real, higher-severity issue than expected.** Diffed both pages against their pre-2026-08-26 versions. Both previously carried an explicit amber "Draft template... has not been reviewed by an attorney — please have counsel review before treating it as \[official/binding\]" warning. **That warning is gone in the current versions** (direct maintainer commits, 2026-08-26, not through a PR), replaced with full, formal, specific legal documents: `PrivacyPage.tsx` now covers cookies/device storage, retention periods by data category, a children's-privacy/COPPA-style clause, and a disclosure-categories list including an ownership-transfer clause; `TermsPage.tsx` now has a "Disclaimer of warranties" section, a "Limitation of responsibility" section (excludes indirect/consequential/punitive damages), an acceptable-use policy, and moderation/suspension/removal rights. Neither page states that an attorney actually reviewed this text. Given StreetRise's booking/question flow routinely carries health, disability, domestic-violence, and family-status information, this is a real legal-exposure question, not just a docs-accuracy one. Not fixed — flagged directly to the maintainer; needs their call on whether this was actually reviewed and, if not, whether to restore the disclaimer or get it reviewed.
- `AccessibilityPage.tsx` also got a full rewrite in the same commit — lower stakes (an accessibility statement inherently hedges, and this one explicitly says "this statement should not be understood as a certification"), but it did silently bump the stated target from WCAG 2.1 to WCAG 2.2 Level AA. Worth confirming that's an intentional, informed choice and not just carried over from whatever generated the new text.
