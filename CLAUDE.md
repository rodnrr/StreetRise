...
- Trust fields (migration 010): `confidence_score` (0–100), `stale_after_days`, `last_provider_update_at`, `last_verified_at`. `stale_after_days` default raised 30 → 90 by migration 040 (not yet applied to live — `docs/apply-migration-040.md`). As of 2026-09-03 the freshness this drives is **internal-only** (provider portal + admin) — the public "may be outdated" badge was removed from the map card, detail sheet, and `/resources/:id`; see Known Open Items.
