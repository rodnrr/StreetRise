...
Migration 035 adds provenance: `external_id`, `source_url` (the public "get involved / careers" page the listing came from), `source_type` (`provider_posted | seeded | agent_assisted`), `last_verified_at`, `last_verify_status` (`confirmed | changed | gone | unreachable`). The 17 seeded rows are backfilled with theirs. `source_url` is what the agent re-reads; a listing with none is skipped.
