...
- **The blog publisher Worker exists on Cloudflare, is connected to the repo via Workers Builds, but has never successfully deployed real code** (`workers/blog-publisher/`). Verified live 2026-08-25: a Worker named `streetrise-blog-publisher` was created on 2026-08-21 (shortly after PRs #81/#82 merged), and `workers_get_worker_code` shows its deployed script is still Cloudflare's stock `"Hello world"` template — the actual source under `workers/blog-publisher/src/` has never been pushed to it. **This is not a missing Git integration**: pushing an unrelated one-line `CLAUDE.md` change on PR #88 triggered a "Workers Builds: streetrise-blog-publisher" GitHub check on that PR, so Workers Builds is wired to the repo and fires on every push (not scoped to `workers/blog-publisher/` paths, at least not effectively). That check **failed in ~0 seconds** (build ID `d5bed1c7-d1e8-4c46-86aa-4dbe7ef23ced`, see Cloudflare dashboard → Workers & Pages → streetrise-blog-publisher → builds). **Root cause confirmed 2026-08-25** from the build log the maintainer pasted in: Workers Builds is running against the **repo root**, not `workers/blog-publisher/`. The log shows `npm clean-install` installing the main app's 621 packages, `npm run build` running the main app's `tsc && vite build` (the log's own output shows `dist/assets/MapPage-*.js`, `AdminBlog-*.js`, etc. — the SPA, not the Worker), then `npx wrangler versions upload` failing with `Missing entry-point to Worker script` because it ran from repo root where there is no `wrangler.jsonc`. This is exactly the config `docs/deploy-blog-worker.md` step 3 specifies and the dashboard's auto-detected defaults evidently overrode: **Root directory** must be `workers/blog-publisher`, **Build command** empty, **Deploy command** `npx wrangler deploy`. The dashboard's Root Directory setting proved unreliable — it kept reverting to `/` even after repeated saves, so the **active fix is no longer the Cloudflare dashboard**: `.github/workflows/deploy-blog-worker.yml` (added 2026-08-25) deploys the Worker via `npm run worker:blog:deploy` from GitHub Actions, which isn't subject to that dashboard misconfiguration. It fires automatically on a push to `main` touching `workers/blog-publisher/**` or the workflow file itself, or on demand via `workflow_dispatch`. It needs two repo secrets (`CLOUDFLARE_API_TOKEN` scoped to Workers Scripts: Edit, and `CLOUDFLARE_ACCOUNT_ID`) that were not yet added as of this writing — **so the Worker was still serving the placeholder script as of 2026-08-25**. To check current status: look at the Actions run history for "Deploy Blog Publisher Worker" (did it run, did it succeed) and independently verify the deployed script via the Cloudflare dashboard or `workers_get_worker_code` — don't assume success from this note without checking. The dashboard runbook (`docs/deploy-blog-worker.md`) is kept as a fallback and explains the Root Directory failure mode in more detail. Until a deploy succeeds and `VITE_BLOG_WORKER_URL` is set in Cloudflare Pages, the AI Draft panel on `/admin/blog` stays hidden and the feature is inert. Note for future sessions: a Claude Code remote session cannot deploy this itself or read Cloudflare build logs directly — the sandbox's egress policy blocks direct calls to `api.cloudflare.com` (confirmed 403 from the agent proxy), and the Cloudflare MCP connector available here is read-only for Workers (list/get/get-code, no deploy, no build-log access, no secret-write). GitHub Actions runs (including this workflow) aren't behind that restriction.
