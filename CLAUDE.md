...
- **The transportation assistance layer is live — migrations 041–048 all applied 2026-09-03.** Re-verified by reading live, not `schema_migrations`: 6 `transportation` resources (2 providers, PSTA and HART), 11,091 transit stops, 184 routes. This closed the same gap migration 036 closed for `clothing` — before it, `SELECT count(*) FROM resources WHERE category = 'transportation'` returned 0, so the `transportation` need chip matched nothing and `isUsefulOption()` hid it. Three things to know. (1) **All six rows are `pending` and some seeded facts are still uncorroborated.** Migration 047 fixed the phone numbers and links against the agencies' own pages, but the "eight designated transit locations" figure for PSTA Direct Connect and both street addresses were never checked; every description says to confirm with the agency. Do not flip a row to `verified` without a real check. (2) **These rows deliberately never reach the map** — `lat`/`lng` NULL, `is_map_ready = FALSE`, `access_type = 'phone_intake'` — because a countywide paratransit or fare programme has a service area, not a doorway, and because nothing was geocoded. So the `transportation` chip on `/map` is still empty; the listings surface through `/transportation`, the "Get There" panel, the homepage grid and the footer. Coverage is **Pinellas + Hillsborough only**; Orlando (LYNX / ACCESS LYNX) and Miami-Dade (Golden Passport, STS, Go Connect) are equivalent and unseeded. (3) **One re-run of 045 is outstanding** — see the Database Migrations section and the top of `docs/apply-migrations-042-046.md`.
...
docs/
  ...
  apply-migration-041.md    # Hand-apply runbook (041 = transportation assistance seed,
                            # then 047 which corrects its phone numbers and links) —
                            # APPLIED live 2026-09-03. Never run 041 without 047 —
                            # 041 alone ships a wrong phone number.
  apply-migrations-042-046.md # Hand-apply runbook (042 = transit tables DDL;
                            # 043/044/045/046 = HART / MCAT / Miami-Dade / GoPasco
                            # GTFS loads + transit backfill; 048 = transit RLS
                            # cleanup, run last) — APPLIED live 2026-09-03;
                            # 045 needs ONE re-run, see the runbook's top note
...
Static GTFS data (migrations 042–046, **applied live 2026-09-03**). One row per stop with
...
**Data — all of this is live as of 2026-09-03.** Migrations 041 + 047
(`docs/apply-migration-041.md`) seed the PSTA and HART assistance programmes;
042–046 + 048 (`docs/apply-migrations-042-046.md`) add the transit tables and
four counties' stops. Verified by reading live: **11,091 stops, 184 routes, 35
feed-derived `public_transit_accessible` flags, 6 transportation listings** (all
`pending` and deliberately off-map). So `/transportation` is no longer an empty
state and the `transportation` need chip is no longer permanently hidden by
`isUsefulOption()` — though the six rows stay off `/map`, because a countywide
programme has a service area, not a doorway.

**One correction is still outstanding on live**: 045 was regenerated after it
was applied, so `mdt:7836` and `mdt:9656` each still credit one route that
neither boards nor alights there. Re-running 045 in full fixes it and is safe —
see the note at the top of `docs/apply-migrations-042-046.md`.

Adding a fifth agency is `npm run transit:sql -- <gtfs-dir> <slug>`, a new
migration from its output, and one line in `AGENCY_BY_COUNTY` — not a schema
change. **PSTA (Pinellas) and LYNX (Orange/Osceola) are the biggest remaining
gaps**: Orlando alone has 31 listings and Pinellas about 30, and neither gets a
nearest-stop line today. Assistance-programme coverage is Pinellas +
Hillsborough only; the Orlando and Miami-Dade equivalents are unseeded, so the
finder tells an Orlando visitor these programmes serve a different area rather
than showing a false match.
...
replaces the standalone-directory design in PR #107, which is kept open as a
reference and should be **closed, not merged** — its files reuse the numbers
056–058 for entirely different SQL.
